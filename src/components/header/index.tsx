...
const Header = () => {
  const { logged, currentPage, darkTheme } = useAppSelector<IUsers>(
    (state) => state.users
  );
  const { isOpenUnderDevInfo } = useAppSelector<IPortal>(
    (state) => state.portal
  );
  ...
  return (
    <div
      className='header'
      style={{ backgroundColor: bgCustomColor(currentPage) }}
      id='headerId'
    >
      <div className='logo'>
      ...
      {!isOpenUnderDevInfo && (
        <>
          <Link to={customizeLink(currentPage, logged)} className='headerLink'>
            <span>{customizeLinkTitle(currentPage, logged)}</span>
          </Link>
          <Link
            to={customizeLink(currentPage, 'restaurant')}
            className='headerRestaurantLink'
          >
            <span>{customizeLinkTitle(currentPage, 'restaurant')}</span>
          </Link>
          <a href='https://alexthunders.ru'>
            <i className='fas fa-home'></i>
          </a>
          <div
            className='goback-btn'
            onClick={() => navigate(-1)}
            data-testid='gobacktestID'
          >
          ...
          <div
            ...
            style={
              darkTheme
                ? { backgroundColor: '#94bbe2', color: '#002259' }
                : { backgroundColor: '#002259', color: 'white' }
            }
          >
