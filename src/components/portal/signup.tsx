...
const SignUpPortal: React.FC = () => {
  ...
  const validatePass = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (pattern.test(e.target.value)) {
      ...
      if (password.length < 8) {
        setPassError("Password shouldn't be less than 8 signs.");
      }
      if (password.length >= 25) {
        setPassError("Password shouldn't exceed 25 signs.");
      }
    } else {
  ...
  return ReactDom.createPortal(
    <div className='authPortal'>
      ...
      <h5 className='error'>
        {error && error} {loginError} {passError}
      </h5>
      <form onSubmit={addUser}>
        <label is='3dx' htmlFor='login'>
          Name:{' '}
        </label>
        <br />
        <input
