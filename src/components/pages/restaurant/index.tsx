...
const Restaurant = () => {
  ...
  return (
    <div
      className={darkTheme ? 'restaurant night-theme' : 'restaurant day-theme'}
    >
      <MealNav />
      <h1>Restaurant</h1>
      <p className='mainInspirationPar'>
        Looking for inspiration on how to cook, what to cook, then you are at
        the right place
      </p>
      <div className='meal-container'>
        {meal && meal.map((meal) => <Meal key={uuid()} meal={meal} />)}
      </div>
      <footer>
        Alex Thunders <i className='fas fa-copyright'></i> 2022
      </footer>
    </div>
  );
};
